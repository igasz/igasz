<h1 align="center">I'm Iga</h1>

<h3 align="center">Computer Science Student at AGH University of Krakow | Software Developer</h3>

<p align="center">
  <a href="https://www.linkedin.com/in/iga-szaflik-590b8b384/">
    <img src="https://img.shields.io/badge/LinkedIn-blue?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn Badge"/>
  </a>
  <a href="mailto:igaszaf@gmail.com">
    <img src="https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white" alt="Email Badge"/>
  </a>
  <a href="https://igasz.github.io/my-sites/portfolio/">
    <img src="https://img.shields.io/badge/Portfolio-252525?style=for-the-badge&logo=google-chrome&logoColor=white" alt="Portfolio Badge"/>
  </a>
</p>

---

### About Me
I'm currently studying **Computer Science** at **AGH University of Krakow**.



### Projects
* **[Darwin World](https://github.com/igasz/Darwin_World_in_Java.git)** - A Java-based evolutionary engine simulating natural selection and population dynamics in a 2D ecosystem.
* **[Online Shop in React](https://github.com/igasz/online_shop_react.git)** - Full-stack web store with JWT authentication, custom CRUD for reviews, and external API integration.
* **[F1 Data Analysis](https://github.com/igasz/f1-data-analysis-python.git)** - Python project utilizing the FastF1 library to explore driver performance and tire strategies.
* **[F1 status](https://github.com/igasz/F1status.git)** - Command Line Interface (CLI) tool written in Python that fetches the current Formula 1 standings and upcoming race information directly in terminal.
* **[heart disease eda](https://github.com/igasz/heart-disease-eda.git)** - An exploration of cardiological data using Python (Pandas, Matplotlib). This project goes beyond basic visualizations by combining multivariate analysis with medical domain knowledge. It uncovers statistical anomalies and extracts clinical insights.
* **[FEM Gravity Potential Solver](https://github.com/igasz/FEM_Gravity_Potential_Solver.git)** - Python app calculating gravitational potential using Gauss-Legendre quadrature.
* **[Factory SQL Database](https://github.com/pawelmalkowski4/BazyDanych.git)** - Advanced T-SQL relational database system for managing manufacturing workflows.
* **[Smart Pantry (Zero-Waste System)](https://github.com/kassimma123/BD_2_Piskorz_Szaflik)** - Full-stack application managing restaurant inventory to minimize food waste. Developed a robust backend using Python (FastAPI) to expose REST APIs and manage complex Oracle DB logic (transactions, triggers, PL/SQL). Built the frontend in React, showcasing strong system architecture and data-handling skills.
* [My Sites](https://github.com/igasz/my-sites.git)
  * **Formula 1 site** - A multi-page enthusiast website dedicated to F1, featuring a dynamic race calendar, multimedia gallery, and an interactive contact form.
  * **Flappy Bird** - A web-based recreation of the classic arcade game, built with JavaScript to handle physics, collision detection, and scoring.


